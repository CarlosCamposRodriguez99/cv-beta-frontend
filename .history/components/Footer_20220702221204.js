import Link from 'next/link';
import Image from 'next/image';
import styles from '../styles/Footer.module.css'

const Footer = () => {

  return (
    <footer className={styles.footer}>
      <div className={`contenedor ${styles.contenido}`}>
          <nav className={styles.navegacion}>
            <Link href="/">
              <a>
                <Image layout='fixed' width={40} height={40} src="/img/home.jpg" alt='home' />
              </a>
            </Link>
            <Link href="/blog">
              <a>
                <Image layout='fixed' width={40} height={40} src="/img/tarea.png" alt='tareas' />
              </a>
            </Link>
            <Link href="/lenguajes">
              <a>
                <Image layout='fixed' width={60} height={60} src="/img/cap.png" alt='cap' />
              </a>
            </Link>
            <Link href="/contacto">
              <a>
                <Image layout='fixed' width={40} height={40} src="/img/contacto.png" alt='contact' />
              </a>
            </Link>
          </nav>

          &nbsp;

          <nav className={styles.navegacion}>
            <li className={styles.list}>
              <Link
                href="https://www.instagram.com/luis_camposr"
              >
                <a
                  target='_blank'
                  className={styles.iconoInsta}
                >
                </a>
              </Link>

              <Link
                href="https://www.facebook.com/"
              >
                <a 
                  className={styles.iconoFace}
                  target='_blank'
                >
                </a>
              </Link>

              <Link
                href="https://web.whatsapp.com/+523331308335"
              >
                <a 
                  className={styles.iconoWhats}
                  target='_blank'
                ></a>
              </Link>

              <Link
                href="https://www.linkedin.com/"
              >
                <a 
                  className={styles.iconoLinke}
                  target='_blank'
                ></a>
              </Link>
            </li>
          </nav>
          
      </div>
    </footer>
  )
}

export default Footer;
